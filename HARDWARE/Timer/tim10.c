...
{		
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM10, ENABLE);//TIM13时钟使能 
  ...
  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化
	...
	GPIO_PinAFConfig(GPIOF,GPIO_PinSource6,GPIO_AF_TIM10); //GPIOA6复用为TIM13
  ...
  TIM10_ICInitStructure.TIM_ICFilter = 0x03;//IC1F=0003 8个定时器时钟周期滤波
  TIM_ICInit(TIM10, &TIM10_ICInitStructure);//初始化定时器2输入捕获通道
	
	TIM_ITConfig(TIM10,TIM_IT_Update|TIM_IT_CC1,ENABLE);//允许更新中断 ,允许CC1IE捕获中断	
  TIM_Cmd(TIM10,ENABLE ); 	 	//使能定时器1
...
u8 	RmtSta_tim10=0;	  	  
u16 Dval_tim10;		//下降沿时计数器的值
u32 RmtRec_tim10=0;	//红外接收到的数据	   		    
u8  RmtCnt_tim10=0;	//按键按下的次数	 
...
{        
    ...
    u8 t1,t2;  
	u32 temp_rmt_rec = 0;
	if(RmtSta_tim10&(1<<6))//得到一个按键的所有信息了
	{ 
	    t1=RmtRec_tim10>>24;			//得到地址码
	    t2=(RmtRec_tim10>>16)&0xff;	//得到地址反码 
