...
{		
  ...
  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化
	...
	GPIO_PinAFConfig(GPIOF,GPIO_PinSource7,GPIO_AF_TIM11); //GPIOA6复用为TIM13
  ...
  TIM11_ICInitStructure.TIM_ICFilter = 0x03;//IC1F=0003 8个定时器时钟周期滤波
  TIM_ICInit(TIM11, &TIM11_ICInitStructure);//初始化定时器2输入捕获通道
	
	TIM_ITConfig(TIM11,TIM_IT_Update|TIM_IT_CC1,ENABLE);//允许更新中断 ,允许CC1IE捕获中断	
  TIM_Cmd(TIM11,ENABLE ); 	 	//使能定时器1
...
u8 	RmtSta_tim11=0;	  	  
u16 Dval_tim11;		//下降沿时计数器的值
u32 RmtRec_tim11=0;	//红外接收到的数据	   		    
u8  RmtCnt_tim11=0;	//按键按下的次数	 
...
{        
    ...
    u8 t1,t2;  
	u32 temp_rmt_rec = 0;
	if(RmtSta_tim11&(1<<6))//得到一个按键的所有信息了
	{ 
	    t1=RmtRec_tim11>>24;			//得到地址码
	    t2=(RmtRec_tim11>>16)&0xff;	//得到地址反码 
...
//	        if(t1==(u8)~t2)
//						sta=t1;//键值正确	 
					sta = RmtRec_tim11 & (0xFF);
					temp_rmt_rec = RmtRec_tim11;
		}   
//		if((sta==0)||((RmtSta_tim1&0X80)==0))//按键数据错误/遥控已经没有按下了
		{
		 	RmtSta_tim11&=~(1<<6);//清除接收到有效按键标识
